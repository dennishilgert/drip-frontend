...
import {
  ...
  ILookupIdentity,
  ILookupIdentityData,
  IUpdateIdentityLocationData
} from 'src/app/models/identity.model'
...
export class IdentityService {
  ...
  constructor(private stateService: StateService, private apiService: ApiService, private toastService: ToastService) {}

  async init(): Promise<IIdentity> {
    console.log('Identity creation ...')

    const identityPromise: Promise<IIdentity> = this.createIdentity()
    identityPromise.then((identity: IIdentity) => {
  ...
  async lookupIdentity(input: ILookupIdentityData): Promise<ILookupIdentity> {
    const requestPromise: Promise<ILookupIdentity> = this.apiService.doGetRequest<ILookupIdentity>(
  ...
  async updateLocation(input: IUpdateIdentityLocationData): Promise<IUpdateIdentityLocationData> {
    const requestPromise: Promise<IUpdateIdentityLocationData> =
      this.apiService.doPatchRequest<IUpdateIdentityLocationData>(
        '/identities/geolocation',
        input,
        new HttpHeaders({ Authorization: `Bearer ${this.getIdentityUuid}` })
      )
    requestPromise
      .then(() => {
