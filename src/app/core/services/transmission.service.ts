...
import {
  ICombinedTransmission,
  IFileTransmission,
  IMessageTransmission,
  ITransmission,
  ITransmissionConfirmation
} from 'src/app/models/transmission.model'
...
export class TransmissionService {
  ...
  listenForTransmissions(): void {
    this.socketService.injectListener(SocketEvent.MESSAGE_TRANSMISSION, (data: string) => {
      ...
      this.retrieveMessageTransmission(messageTransmission).then((message: string) => {
    ...
    this.socketService.injectListener(SocketEvent.FILE_TRANSMISSION, (data: string) => {
      ...
      this.retrieveFileTransmission(fileTransmission).then(() => {
  ...
  async transmitMessage(toName: string, message: string): Promise<ITransmissionConfirmation> {
    const requestPromise: Promise<ITransmissionConfirmation> = this.apiService.doPostRequest(
      '/transmissions/message',
      { toName, message },
      new HttpHeaders({ Authorization: `Bearer ${this.identityService.getIdentityUuid}` })
    )
  ...
  async retrieveMessageTransmission(messageTransmission: IMessageTransmission): Promise<string> {
    const requestPromise: Promise<string> = this.apiService.doGetRequest<string>(
      `/transmissions/message/${messageTransmission.uuid}`,
      {
  ...
  async retrieveFileTransmission(fileTransmission: IFileTransmission): Promise<Blob> {
    const requestPromise: Promise<Blob> = this.apiService.doGetRequest<Blob>(
      `/transmissions/file/${fileTransmission.uuid}`,
      {
        headers: new HttpHeaders({ Authorization: `Bearer ${this.identityService.getIdentityUuid}` }),
